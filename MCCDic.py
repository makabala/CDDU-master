...
import torch
import numpy as np
from MCM import SAB, JRFAB_same_scale
from  RFormer import Illumination_Estimator
    ...
    def forward(self, x, y, z):
        ...
        for idx in range(1, self.ista_num_steps):
            ...
            for i in range(self.num_layers):
                ...
                x_list[i] = self.resnet_x[i](self.relu(x_list[i]),adj_err_list_x[i])
                y_list[i] = self.resnet_y[i](self.relu(y_list[i]),adj_err_list_y[i])
            ...
            for i in range(self.num_layers):
                ...
                z_list[i] = self.resnet_z[i](self.relu(z_list[i]),adj_err_list_z[i])
...
class decoder(nn.Module):
    def __init__(self, in_channel, channel_fea):
        ...
        self.conv_1 = nn.Conv2d(in_channels=self.filters , out_channels=self.filters, kernel_size=self.kernel_size,
...
class MCCDic(nn.Module):
    def __init__(self,dim=64,gps=3,num_path=2):
        ...
        self.dim = dim
        self.gps = gps
        self.ill = Illumination_Estimator()
        self.Hsv = RGB2HSV()

        self.layer_in_y = nn.Conv2d(self.in_channel, self.channel_fea, 3, 1, 1)
        self.layer_in_z = nn.Conv2d(self.channel_fea * 2, self.channel_fea, 3, 1, 1)

        self.pedict_ista = ista_unet(kernel_size=3, hidden_layer_width_list=[128, 96, 64], n_classes=self.channel_fea,
                                     ista_num_steps=3)
    ...
    def forward(self, x):
        ...
        y_in = self.layer_in_y(hsv)
        z_in = self.layer_in_z(torch.cat([ill_fea, y_in], 1))
        # iteration
        x_unique, y_unique, z_common = self.pedict_ista(ill_fea, y_in, z_in)
        # reconstruction


        w = self.fusion(torch.cat([x_unique, y_unique,z_common], dim=1))
        w = torch.split(w, 1, dim=1)
        x4 = w[0] * x_unique + w[1] * y_unique + w[2] * z_common
        out = self.decoder(x4)
        return out
